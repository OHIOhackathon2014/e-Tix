...
using OSUTicketExchange.Web.BL;
...
namespace OSUTicketExchange.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [AllowAnonymous]
        public ActionResult FacebookLogin(UserInfo uI)
        {
            var success = false;
            if (uI!=null)
            {

                success = AuthSystem.Instance.Authorize(uI);

            }
            if (success)
            {

                //Check to see if the user exists in the database or not and if so updates the user info
                var u = new UserBL().UserLogin(uI);
                if (u != null)
